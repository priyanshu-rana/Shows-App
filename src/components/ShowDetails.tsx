    ...
    <div className="bg-black h-screen ">
      <div className="flex space-x-8 p-5  ">
        <div className="space-y-4 h-full ">
          <img
            className="w-80 border-4 border-gray-500"
            src={
              show.image?.original ||
          ...
          <h1 className="text-lg font-bold text-white">
            Language- <span className=" text-green-600">{show.language}</span>
          </h1>
          <h1 className="text-lg font-bold text-red-500 ">
            Rating -
            {(show.rating.average && show.rating.average) || "not available"}
          </h1>
        ...
        <div className="border-2 border-gray-400 w-full h-full p-3">
          <h1 className="font-bold text-2xl text-gray-200">{show.name}</h1>
...
const mapStateToProps = (s: State, props: any) => {
  return { show: showEntitiesSelector(s)[+props.params.showId] };
};
